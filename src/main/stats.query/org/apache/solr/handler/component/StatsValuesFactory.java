...
//  private Long totalCount, offset, limit;
//  protected String statsQuery;
  
  protected Map<String, StatsValues> statsQueryMap = new HashMap<String, StatsValues>();

  // facetField   facetValue
  protected Map<String, Map<String, StatsValues>> facets = new HashMap<String, Map<String, StatsValues>>();

//  protected Map<String, Object> facetsOverall = new HashMap<String, Object>();
  ...
  public void accumulate(NamedList stv) {
    ...
    // elements like count may not exist, 
    ...
    NamedList queriesNL = (NamedList) stv.get(StatsComponent.STATS_QUERIES);
    if (queriesNL != null) {
      Iterator<Map.Entry<String,T>> it = queriesNL.iterator();
      while(it.hasNext())
      {
        Map.Entry<String,T> entry = it.next();
        String query = entry.getKey();
        StatsValues queryStv = statsQueryMap.get(query);
        if(queryStv==null)
        {
          queryStv = StatsValuesFactory.createStatsValues(sf);
          statsQueryMap.put(query, queryStv);
        }
        queryStv.accumulate((NamedList) queriesNL.get(query));
      }
    }
    ...
    for (int i = 0; i < f.size(); i++) {
      ...
      for (int j = 0; j < vals.size(); j++) {
        ...
        if("totalCount".equals(val) || "offset".equals(val) || "limit".equals(val)){
          continue;
        }
  ...
  public NamedList<?> getStatsValues() {
    ...
    if(!statsQueryMap.isEmpty())
    {
      NamedList<NamedList<?>> nl = new SimpleOrderedMap<NamedList<?>>();
      for (Map.Entry<String,StatsValues> entry : statsQueryMap.entrySet()) {
        nl.add(entry.getKey(), entry.getValue().getStatsValues());
      }
      res.add(StatsComponent.STATS_QUERIES, nl);
    }
    // add the facet stats
